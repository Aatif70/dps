...
class _AdminDivisionsScreenState extends State<AdminDivisionsScreen> {
  ...
  Timer? _debounce;
  List<DivisionItem> _all = [];
  List<DivisionItem> _filtered = [];
  bool _loading = true;
  int _page = 1;
  ...
  Future<void> _load() async {
    ...
    final data = await AdminClassesService.fetchDivisions();
    if (!mounted) return;
    setState(() {
      _all = data;
      _applyFilter();
  ...
  void _applyFilter() {
    ...
    List<DivisionItem> base = _all;
    if (q.isNotEmpty) {
      base = base.where((e) => e.divName.toLowerCase().contains(q) || e.classId.toString().contains(q)).toList();
    }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Divisions', style: TextStyle(fontWeight: FontWeight.bold, color: Color(0xFF2D3748))),
                                  ...
                                  children: [
                                    ...
                                    Expanded(
                                      child: Column(
                                        ...
                                        children: [
                                          Text(item.divName, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(fontWeight: FontWeight.w700, fontSize: 16, color: Color(0xFF1E293B))),
                                          const SizedBox(height: 6),
                                          Text('ClassId: ${item.classId} • Incharge: ${item.empId}', style: const TextStyle(color: Color(0xFF64748B))),
                                        ],
                                      ),
                                    ),
                                  ],
