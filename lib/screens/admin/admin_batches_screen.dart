...
class _AdminBatchesScreenState extends State<AdminBatchesScreen> {
  ...
  Future<void> _load() async {
    ...
    final data = await AdminClassesService.fetchBatches();
    if (!mounted) return;
    setState(() {
      _all = data;
      _applyFilter();
  ...
  void _applyFilter() {
    ...
    List<BatchItem> base = _all;
    if (q.isNotEmpty) {
      base = base.where((e) => e.batchName.toLowerCase().contains(q) || e.courseYear.toString().contains(q)).toList();
    }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Batches', style: TextStyle(fontWeight: FontWeight.bold, color: Color(0xFF2D3748))),
                                ...
                                child: Row(
                                  children: [
                                    Container(
                                      ...
                                      child: const Icon(Icons.layers_rounded, color: Color(0xFF58CC02)),
                                    ),
                                    ...
                                    Expanded(
                                      child: Column(
                                        ...
                                        children: [
                                          Text(item.batchName, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(fontWeight: FontWeight.w700, fontSize: 16, color: Color(0xFF1E293B))),
                                          const SizedBox(height: 6),
                                          Text('ClassMasterId: ${item.classMasterId} • Year: ${item.courseYear}', style: const TextStyle(color: Color(0xFF64748B))),
                                        ],
                                      ),
                                    ),
                                  ],
  ...
  Widget _buildPager() {
    final total = _search.text.trim().isEmpty ? _all.length : _all.where((e) => true).length;
